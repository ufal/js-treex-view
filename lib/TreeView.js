...
var styles = require('./TreeView.less');
...
(function (twproto) {
  twproto.init = function (document) {
    ...
    self.desc = null;
    self.svg = top
      .append('div').attr('class', styles.treeviewGfx)
      .append('svg');
  };
  ...
  twproto.attributes = function (container) {
    this.attrs = d3.select(container);
  };
  ...
  twproto.drawBundle = function () {
    ...
    var trees = svg.selectAll('.' + styles.tree)
      .data(bundle.allTrees(), function (d) {
        return d.language + '-' + d.layer;
      });
    ...
    trees.each(function (d) {
      ...
      !self.select('g.' + styles.link).empty() || self.append('g').attr('class', styles.links);
      !self.select('g.' + styles.link).empty() || self.append('g').attr('class', styles.nodes);

      var link = self.select('g.' + styles.links).selectAll('.' + styles.link)
      ...
      var node = self.select('g.' + styles.nodes).selectAll('.' + styles.node)
      ...
      if (desc) {
        mouseout = function (d) {
          desc.selectAll('span.' + d.id).classed(styles.highlight, false);
          if (d.hint) {
            hint.hide();
          }
        };
        ...
        node
          .on('mouseover', function (d) {
            desc.selectAll('span.' + d.id).classed(styles.highlight, true);
            if (d.hint) {
              hint.show(d.hint);
            }
          })
  ...
  function displayDesc(desc, top, bundle) {
    ...
    sentence.enter().append('span')
      .attr('class', function (d) {
        return d.slice(1).map(function (itm) {
          return styles[itm] ? styles[itm] : itm
        }).join(' ');
      })
      .each(function (d) {
        var self = d3.select(this),
        ...
        self.classed(styles.mouseHighlight, true);
